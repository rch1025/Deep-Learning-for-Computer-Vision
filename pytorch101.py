...
def multiples_of_ten(start, stop):
  ...
  try:
      num_list = torch.tensor(num_list).to(torch.float64)
  except:
      num_list = torch.empty(0)
  x = num_list
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def make_one_hot(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y = torch.eye(max(x)+1, dtype=torch.int64)
  idx = torch.tensor(x)
  y = y[idx, :]
  #############################################################################
  #                            END OF YOUR CODE                               #
