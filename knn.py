...
def compute_distances_two_loops(x_train, x_test):
  ...
  ## train 데이터와 test 데이터의 각 픽셀 값의 차이의 제곱
  ...
  for i in range(num_train):
    for j in range(num_test):
      train_flatten = x_train[i].reshape(-1)
      test_flatten = x_test[j].reshape(-1)
      sub = train_flatten.sub(test_flatten)
      # 제곱 유클리디안 거리
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_f = x_train.reshape(num_train, -1)
  x_test_f = x_test.reshape(num_test, -1)
  dists = -2 * torch.mm(x_train_f, x_test_f.t()) + torch.sum(x_test_f**2, dim=1) + torch.sum(x_train_f**2, dim=1).reshape(-1, 1)
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  values, index = torch.topk(dists, k=3, dim=0, largest=False)
  y_pred = y_train[index[0, :]]
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
